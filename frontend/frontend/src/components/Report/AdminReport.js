...
const AdminReport = () => {
  const [clinicChoice, setClinicChoice] = React.useState("");

  const handleChange = (event) => {
    setClinicChoice(event.target.value);
  };
  ...
  const [adminReviewChart, setAdminReviewChart] = useState({
  ...
  const generate = async () => {
    try {
      const response_checkins = await axios.get(
        `${serverURL}/report/admin/${clinicChoice}?startDate=${moment(
          startDate
        ).format("YYYY-MM-DD")}&endDate=${moment(endDate).format(
          "YYYY-MM-DD"
        )}&systemDate=${moment(systemTime).format("YYYY-MM-DD")}`
      );
      setError("");
  ...
  const AdminReviewDoughnut = () => (
    <Doughnut
      data={adminReviewChart}
      options={{
        title: {
          display: true,
  ...
  return (
    <>
      ...
      <Grid container>
        ...
        <Grid item xs={1}></Grid>
        <Grid item xs={3}>
          <Item>{AdminReviewDoughnut()}</Item>
          <Item>
            <Typography>Admin Report</Typography>
          </Item>
        </Grid>
        <Grid item xs={1}></Grid>
        <Grid item xs={4}>
